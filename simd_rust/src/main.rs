...
fn scalar_add_non_optimizable(a: &[f32], b: &[f32], result: &mut [f32]) {
    for i in 0..a.len() {
        result[i] = std::hint::black_box(a[i]) + std::hint::black_box(b[i]);
    }
}
...
async fn async_parallel_add_non_optimizable(
    a: Arc<Vec<f32>>,
    b: Arc<Vec<f32>>,
    ...
    for t in 0..NUM_TASKS {
        ...
        handles.push(tokio::spawn(async move {
            let partial: Vec<f32> = (start..end)
                .map(|i| std::hint::black_box(a[i]) + std::hint::black_box(b[i]))
                .collect();
...
async fn main() {
    ...
    bench("scalar_add_non_optimizable:", || {
        scalar_add_non_optimizable(&a, &b, &mut result)
    });
    ...
    bench("async_parallel_add_non_optimizable:", || {
        tokio::task::block_in_place(|| {
            handle.block_on(async_parallel_add_non_optimizable(
                Arc::clone(&a),
                Arc::clone(&b),
                Arc::clone(&async_result),
            ));
